...
function getMasterKeyFromKeychain(): string | null {
  try {
    const fromPorch = getPasswordSync(KEYCHAIN_SERVICE, KEYCHAIN_ACCOUNT);
    if (fromPorch) return fromPorch;
  } catch { /* ignore */ }
  try {
...
function encrypt(plain: string, key: Buffer): EncryptedPayload {
...
function getSecretsPath(): string {
  if (fs.existsSync(SECRETS_PATH)) return SECRETS_PATH;
  if (fs.existsSync(LEGACY_SECRETS_PATH)) return LEGACY_SECRETS_PATH;
...
function loadSecrets(): SecretsMap {
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      const legacy = parsed as SecretsMap;
      if (Object.values(legacy).every((v) => typeof v === 'string')) return legacy;
    }
  } catch {
    /* ignore corruption */
  }
...
export function deriveKeyFromPassword(password: string): string {
 ...
 */
export function storeMasterKeyInKeychain(keyOrPassphrase: string, isPassphrase = true): void {
...
export function deleteMasterKeyFromKeychain(): void {
  try {
...
export function bootstrapSecretsFromKeychain(): void {
  getMasterKey();
}
