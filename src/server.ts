...
async function main() {
  ...
  app.use((req, res, next) => {
    ...
    const mcpName = Object.keys(config.mcps).find(
      (k) => toTunnelSubdomain(k, config.mcps[k]) === subdomain
    );
    if (!mcpName) return next();
  ...
  const tunnelProxyHandler = (req: express.Request, res: express.Response) => {
  ...
  app.patch('/api/mcp/:name/tunnel-subdomain', (req, res) => {
    try {
      ...
      const { tunnelSubdomain } = req.body as { tunnelSubdomain?: string | null };
      const config = loadConfig();
      const mcp = config.mcps[name];
      if (!mcp) return res.status(404).json({ error: 'MCP not found' });
      (mcp as { tunnelSubdomain?: string }).tunnelSubdomain =
      ...
      appendLog({
        ...
        detail: (mcp as { tunnelSubdomain?: string }).tunnelSubdomain || '(default)',
      });
      res.json({ ok: true, tunnelSubdomain: (mcp as { tunnelSubdomain?: string }).tunnelSubdomain });
    } catch (err) {
      res.status(500).json({ error: String(err) });
    }
  });
  ...
  app.get('/api/tunnel/status', (_req, res) => {
    ...
    const subdomainUrls =
      domain && mcpNames.length
        ...
        : null;
    const tunnelSubdomains = Object.fromEntries(
  ...
  app.post('/api/tunnel/stop', (req, res) => {
    try {
      ...
      if (stopped) appendLog({ type: 'tunnel', message: 'Cloudflare tunnel stopped', detail: null });
  ...
  app.post('/api/tunnel/token/:mcpName', (req, res) => {
    try {
      ...
      if (domain) {
        ...
        if (baseUrl) fullUrl = `${baseUrl}/tunnel/${encodeURIComponent(mcpName)}`;
      }
  ...
  app.get('/api/mcp-client/config', (req, res) => {
    try {
      const platform = (req.query.platform as Platform) || detectPlatform();
      const client = req.query.client as Client;
      ...
      if (!result) return res.status(400).json({ error: 'Unknown client' });
      res.json({
  ...
  app.post('/api/mcp-client/install', (req, res) => {
    try {
      const { platform, client } = req.body as { platform?: Platform; client?: Client };
      const plat = (platform || detectPlatform()) as Platform;
