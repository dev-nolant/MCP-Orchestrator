...
import type { OrchestratorConfig, Workflow, WorkflowStep, McpConfig } from './config.js';
import {
  startOrchestratorTunnel,
  stopOrchestratorTunnel,
  getOrchestratorTunnelUrl,
  getOrchestratorTunnelPersisted,
  isCloudflareTunnelActive,
...
import {
  getTunnelTokenMcpNames,
  setTunnelToken,
  deleteTunnelToken,
  generateTunnelToken,
} from './tunnel-tokens.js';
...
async function createMcpServer(): Promise<McpServer> {
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      ...
      if (!w) return { content: [{ type: 'text' as const, text: `Workflow not found: ${name}` }], isError: true };
  ...
  server.registerTool(
    ...
    async ({ name, description, steps, trigger, schedule }) => {
      ...
      if (config.workflows.some((w) => w.name === name)) {
        return { content: [{ type: 'text' as const, text: `Workflow "${name}" already exists` }], isError: true };
      }
      const w: Workflow = { name, description, steps: steps as WorkflowStep[], trigger: trigger ?? 'manual', schedule };
      config.workflows.push(w);
      saveConfig(config);
      startScheduler(config);
      appendLog({ type: 'config', message: `Added workflow "${name}"`, detail: null, success: true });
      return { content: [{ type: 'text' as const, text: `Workflow "${name}" created.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name, description, steps, trigger, schedule }) => {
      ...
      if (idx < 0) return { content: [{ type: 'text' as const, text: `Workflow not found: ${name}` }], isError: true };
      const w = config.workflows[idx];
      if (description !== undefined) w.description = description;
      if (steps !== undefined) w.steps = steps as WorkflowStep[];
      if (trigger !== undefined) w.trigger = trigger;
      if (schedule !== undefined) w.schedule = schedule;
      saveConfig(config);
      startScheduler(config);
      appendLog({ type: 'config', message: `Updated workflow "${name}"`, detail: null, success: true });
      return { content: [{ type: 'text' as const, text: `Workflow "${name}" updated.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      ...
      return { content: [{ type: 'text' as const, text: `Workflow "${name}" deleted.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name, schedule }) => {
      ...
      return { content: [{ type: 'text' as const, text: `Workflow "${name}" scheduled: ${schedule}` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      ...
      return { content: [{ type: 'text' as const, text: `Workflow "${name}" is now manual-only.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async () => {
      const config = loadConfig();
      const entries = Object.entries(config.mcps).filter(([, m]) => (m as { enabled?: boolean }).enabled !== false);
      const results: Record<string, { online: boolean; toolsCount?: number; error?: string }> = {};
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      if (config.mcps[name]) {
        return { content: [{ type: 'text' as const, text: `MCP "${name}" already exists` }], isError: true };
      }
      ...
      if (type === 'stdio') {
        ...
        const url = args.url;
        if (!url) return { content: [{ type: 'text' as const, text: 'url required for type url' }], isError: true };
      ...
      return { content: [{ type: 'text' as const, text: `MCP "${name}" added.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      const config = loadConfig();
      if (!config.mcps[name]) {
        return { content: [{ type: 'text' as const, text: `MCP "${name}" not found` }], isError: true };
      }
      const usedBy = config.workflows.filter((w) => w.steps.some((s) => s.mcp === name));
      if (usedBy.length > 0) {
        return {
          content: [{ type: 'text' as const, text: `Cannot remove: workflow(s) "${usedBy.map((w) => w.name).join(', ')}" use "${name}". Remove or update those workflows first.` }],
          isError: true,
        };
      }
      ...
      return { content: [{ type: 'text' as const, text: `MCP "${name}" removed.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      ...
      if (!mcp) return { content: [{ type: 'text' as const, text: `MCP "${name}" not found` }], isError: true };
      mcp.enabled = true;
      saveConfig(config);
      startScheduler(config);
      appendLog({ type: 'spin', message: `Enabled ${name}`, detail: null, success: true });
      return { content: [{ type: 'text' as const, text: `MCP "${name}" enabled.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      ...
      return { content: [{ type: 'text' as const, text: `MCP "${name}" disabled.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ mcp, tool, args = {} }) => {
      ...
      if (!mcpConfig) return { content: [{ type: 'text' as const, text: `MCP "${mcp}" not found` }], isError: true };
      if ((mcpConfig as { enabled?: boolean }).enabled === false) {
        return { content: [{ type: 'text' as const, text: `MCP "${mcp}" is disabled. Enable it first.` }], isError: true };
      }
  ...
  server.registerTool(
    ...
    async ({ mcp }) => {
      ...
      const entries = mcp
        ...
        : Object.entries(config.mcps).filter(([, m]) => (m as { enabled?: boolean }).enabled !== false);
      const out: Record<string, Array<{ name: string; description: string }>> = {};
      for (const [name, mcpConfig] of entries) {
  ...
  server.registerTool(
    ...
    async () => {
      ...
      const subdomainUrls =
        domain && Object.keys(config.mcps).length
          ? Object.fromEntries(
              Object.keys(config.mcps).map((n) => [
                n,
  ...
  server.registerTool(
    ...
    async () => {
      ...
      return { content: [{ type: 'text' as const, text: `Tunnel started: ${url}` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async () => {
      ...
      if (stopped) appendLog({ type: 'tunnel', message: 'Cloudflare tunnel stopped', detail: null, success: true });
      return { content: [{ type: 'text' as const, text: stopped ? 'Tunnel stopped.' : 'Tunnel was not running.' }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ domain }) => {
      const d = domain.trim();
      if (!d) return { content: [{ type: 'text' as const, text: 'domain required' }], isError: true };
      setSecret('cloudflare_tunnel_domain', d);
      appendLog({ type: 'config', message: 'Set tunnel domain', detail: d, success: true });
      return { content: [{ type: 'text' as const, text: `Tunnel domain set to ${d}` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async () => {
      ...
      return {
        content: [{ type: 'text' as const, text: result.success ? result.message : `Failed: ${result.message}` }],
  ...
  server.registerTool(
    ...
    async ({ mcpName }) => {
      ...
      if (!config.mcps[mcpName]) {
        return { content: [{ type: 'text' as const, text: `MCP "${mcpName}" not found` }], isError: true };
      }
      ...
      return { content: [{ type: 'text' as const, text: JSON.stringify({ mcpName, token, fullUrl }, null, 2) }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ mcpName }) => {
      ...
      return { content: [{ type: 'text' as const, text: `Token revoked for ${mcpName}.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ server: serverArg }) => {
      ...
      if (!serverDetail?.name) {
        return { content: [{ type: 'text' as const, text: 'Missing server data (need name)' }], isError: true };
      }
      ...
      return { content: [{ type: 'text' as const, text: 'No supported package or remote transport found' }], isError: true };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ package: pkg, args: extraArgs = [] }) => {
      ...
      if (!/^@?[\w.-]+\/[\w.-]+$/.test(pkgTrim.replace(/^@/, ''))) {
        return { content: [{ type: 'text' as const, text: 'Invalid npm package. Use format: @org/package or org/package' }], isError: true };
      }
...
function registerResources(server: McpServer): void {
  ...
  server.registerResource(
    'config',
    'orchestrator://config',
    { mimeType: 'application/json', description: 'Full orchestrator config (mcps, workflows)' },
    () => Promise.resolve(configResource()),
  );
  ...
  server.registerResource(
    'status',
    'orchestrator://status',
    { mimeType: 'application/json', description: 'MCP status and tunnel status' },
    statusResource,
  );
  ...
  server.registerResource(
    'logs',
    'orchestrator://logs',
    { mimeType: 'application/json', description: 'Recent logs' },
    () => Promise.resolve(logsResource()),
  );
