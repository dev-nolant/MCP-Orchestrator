...
function saveOrchestratorTunnelState(state) {
    try {
        if (state) {
    ...
    return new Promise((resolve) => {
        ...
        proc.on('exit', (code) => {
            ...
            if (code === 0) {
                ...
                const msg = stderr.includes('You have successfully logged in') || isCloudflareLoggedIn()
                    ? 'Already logged in.'
    ...
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            if (!resolved) {
                resolved = true;
                proc.kill('SIGTERM');
                reject(new Error('cloudflared did not report URL within 30s. Is cloudflared installed? Run: brew install cloudflared'));
            }
        }, 30000);
        const onOutput = (data) => {
        ...
        proc.on('error', (err) => {
            if (!resolved) {
                resolved = true;
                clearTimeout(timeout);
                reject(new Error(`cloudflared failed: ${err.message}. Install with: brew install cloudflared`));
            }
        });
        proc.on('exit', (code) => {
 ...
 */
async function startNamedTunnel(port) {
    const token = getTunnelToken();
    const publicUrl = getNamedTunnelPublicUrl();
    if (!token) {
        throw new Error('Cloudflare tunnel token required. Set CLOUDFLARE_TUNNEL_TOKEN or add cloudflare_tunnel_token to secrets.');
    }
    if (!publicUrl) {
        throw new Error('Cloudflare tunnel public URL required. Set CLOUDFLARE_TUNNEL_PUBLIC_URL or add cloudflare_tunnel_public_url to secrets. This is the hostname you configured in Cloudflare (e.g. https://mcp.example.com).');
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            if (!resolved) {
                resolved = true;
                proc.kill('SIGTERM');
                reject(new Error('cloudflared named tunnel did not connect within 30s. Check your token and ingress config.'));
            }
        }, 30000);
        const onOutput = (data) => {
            const text = data.toString();
            // Named tunnel prints "Connection established" or similar when ready
            if ((text.includes('Connection') && text.includes('established')) ||
        ...
        proc.on('exit', (code) => {
            if (!resolved) {
                resolved = true;
                clearTimeout(timeout);
                reject(new Error(`cloudflared exited with code ${code}. Ensure your tunnel ingress in Cloudflare routes to http://localhost:${port}.`));
            }
        });
 ...
 */
async function startLoggedInTunnel(port, baseDomain, mcps) {
    ...
    const ingressRules = mcps.flatMap(({ name, config }) => {
        const subdomain = toTunnelSubdomain(name, config);
...
export async function startOrchestratorTunnel(port, options) {
...
export function stopOrchestratorTunnel() {
    return stopCloudflareTunnel();
}
export function getOrchestratorTunnelUrl() {
...
export function getOrchestratorTunnelPersisted() {
