...
$task = Get-ScheduledTask -TaskName "Porch" -ErrorAction SilentlyContinue
if (-not $task) { $task = Get-ScheduledTask -TaskName "MCP Orchestrator" -ErrorAction SilentlyContinue }
if ($task) {
